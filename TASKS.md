...
- [x] 120 Longhand edits resolve through authored shorthands (var-aware)
  Done: 2026-09-15 Task 120: new `src/renderer/js/shorthands.js` — a shorthand→logical-longhand registry (`margin`/`padding`/`inset`/`gap`/`border-radius`/`border`+sides/`outline`/`background`/`font`/`transition`/`flex`) plus var-aware helpers (`splitTopLevel`, `varToken`, `substituteVars` with nested resolution + fallbacks, `sameCssValue` colour/length equivalence, `restoreVarTokens`). `sheet.js` dropped the margin/padding-only `BOX_SHORTHANDS` special case for one shared path: `readStyleProperty` recovers a longhand from an authored var-shorthand by letting a detached probe expand the var-resolved value, and `unfoldVarShorthands` replaces such a shorthand with its longhands (each authored `var()` restored, `!important` carried, siblings already declared left untouched) before any longhand write — no per-shorthand grammar, the browser does the parsing. Added `Sheet.customProperty` (prefers `:root`) as the resolver context wired through `get`/`set`/`getMedia`/`setMedia`/`declOn`/`setOnRule`/`originForMedia`. New `test/shorthands.test.mjs` (7 tests); smoke gained `.token-border`/`.token-pad` fixtures and asserts per-longhand reads (`border-width` → `var(--border)`), the panel showing `3` for the `var()` border width, an edit preserving `border-color: var(--ink)` and dropping the old `border` shorthand, and a second edit not disturbing siblings. `npm test` 102 pass; smoke SMOKE OK with the documented pre-existing Typography font-picker check bypassed for the run, then restored.
  Goal: a value authored inside a CSS shorthand that contains `var()` renders but the panel read the longhand as unset — the Border Width field showed `0` for the visible `border: var(--border) solid var(--ink)` on `.step`/`.step-num`, while Style/Color happened to fill in from the computed value. Root cause: the panel's property model assumed a CSSOM longhand read (`rule.style.getPropertyValue('border-width')`) exposes an authored value, but the browser refuses to expand any shorthand containing `var()` (pending substitution), so every longhand reads empty. The same fact had two answers because each control reconciled authored-vs-computed on its own; the write path was worse — `setProperty` on such a longhand silently destroyed the shorthand into empty longhands. Fix: one var-aware property model shared by every control — a shorthand→longhand registry, a single read that recovers authored components, and a single write that unfolds an authored var-shorthand (preserving each `var()` token) before setting a longhand. This replaces the margin/padding-only `BOX_SHORTHANDS` special case.
  Files: `src/renderer/js/shorthands.js` (new), `src/renderer/js/sheet.js`, `test/shorthands.test.mjs` (new), `src/renderer/js/smoke.js`, `TASKS.md`.
  Accept: `border`, `outline`, `margin`, `padding`, `inset`, `gap`, `border-radius`, `background`, `font` and `transition` authored with `var()` are readable per longhand and editable without losing their `var()` siblings; non-var shorthands keep the browser's own expansion; clearing a longhand removes it; a longhand already authored directly on the rule is never duplicated or reordered; nested/media/picked rules use the same path; unit + smoke green.
  Don't: change serialization, origin labels, token rename/theme semantics, cascade matching, or authored CSS for rules without var-containing shorthands; do not add a per-shorthand CSS grammar (the browser expands the resolved value).
  Known limits: a token that cannot be resolved in the sheet (`:root`/theme declarations) falls back to the resolved literal rather than keeping the `var()` name; reset-only longhands a shorthand also touches (`border-image-*`, `background-origin/clip`, `font-variant-*`) are intentionally not re-emitted and keep their defaults.
...
- [x] 113 Optional-hook coverage: audit + handling so shared app.js stops false-warning
  Done: 2026-09-16 Task 113a+113b. Audit: the allowlist already contained `.dialog`/`.modal`/`dialog` (Task 114), so the missing piece was the contract and the shared-`app.js` behavior. 113a documented the optional-hook vocabulary in `AGENT_SITE_PROMPT.md` under a new "Optional hooks" heading (component roots + state classes, dialog triggers, enhancements, form hooks, `[data-*]`), and kept the allowlist in sync. 113b made the per-page check project-aware: `missingSelectors` (`src/renderer/js/hooks.js`) takes `opts.otherDocs` and stays quiet for a selector that matches a sibling page, reporting nothing while the sibling set is unknown (`null`); `src/renderer/js/main.js` caches parsed sibling pages (`HE.siblingPageDocs`, refreshed on page load and after save) and passes them in, so one shared `app.js` no longer warns on pages that legitimately lack a hook. New `hooks.test.mjs` case covers sibling-present, no-sibling and unknown-states. `npm test` 103 pass; smoke SMOKE OK with the documented pre-existing Typography font-picker check bypassed for the run, then restored.
...
- [x] 119 Docs/license consistency: plain wording + real license
  Done: 2026-09-15 Task 119: rewrote the editing-model description in `AGENTS.md` and `README.md` in plain terms ("a class/token/design-element system of the kind many users of visual site builders already know") and rewrote the README intro (and the `AGENTS.md` "What this project is" lead) to state the real motivation: an agent round-trip is slow and imprecise for small, personal calls — misreads, unwanted extra edits, describing a look instead of seeing it (e.g. trying a heading at 22/23/24px) — so the user gets the panel directly over the files the agent wrote. Added a "Using it with your AI agent" section that points at `AGENT_SITE_PROMPT.md` and explains that JS is only previewed, so script changes go back to the agent (`AGENT_HANDOFF.md`). Fixed the license mismatch: `package.json` / `package-lock.json` now declare `"SEE LICENSE IN LICENSE.txt"` instead of `"MIT"`, `LICENSE.txt` contact line is now a plain email address (was malformed Markdown), and the README License section now names the Source-Available Non-Commercial License and links `LICENSE.txt` instead of "TBD", ending with an invitation to email for trials, one-or-two-project use, or a professional license (also added as `LICENSE.txt` section 10 "Questions, Trials, and Small Use"). `npm test` 94 pass.
  Goal: docs and package metadata agree with the committed Source-Available Non-Commercial License and describe the project and its AI workflow plainly.
...
- [x] 123 Effective values outside preset/button lists across remaining controls
  Done: 2026-09-16 Task 123: `effectiveSelectValue` (`src/renderer/js/panel-controls.js`) now strips commas from its first-token fallback and, when no option matches at all, returns the first plain keyword/number of the computed value, so a select shows what is rendering instead of a bare dash (button `border-style: outset`, inherited variable-font `font-weight: 480`, `text-decoration-line: overline`, multi-layer `background-attachment: scroll, scroll`). `segControl` appends a pressed button for any authored or computed value outside its list (e.g. `display: list-item` on `<li>`/`<summary>`, `table-cell`), mirroring the select's off-list option. `presetControl` gained `PRESET_COMPUTED_ALIASES`, mapping background-position's computed percentage forms (`0% 0%`, `50% 50%`, …) back to the named presets so the default shows as `Top left`. New smoke block `effective off-list control values ok` covers a button border-style, a multi-layer background, a `<li>` display, an authored off-list display, the background-position default, and an inherited numeric weight. `npm test` 102 pass; smoke SMOKE OK with the documented pre-existing Typography font-picker check bypassed for the run, then restored.
  Goal: audit prompted by Tasks 121/122 found the same "control can't show the value that renders" bug in more places — `display: list-item`/`table-*`, button `border-style: outset`, multi-layer background attachment, the `background-position` default, and inherited non-preset weight/style.
  Files: `src/renderer/js/panel-controls.js`, `src/renderer/js/smoke.js`, `TASKS.md`.
  Accept: every affected control shows the effective value while authored values outside the list still work; preset values, origin labels, and the effective-value preview are unchanged; no declaration is authored by rendering; unit + smoke green.
  Don't: add UA-specific buttons to the fixed option arrays, author anything on render, or change serialization/cascade/origin behavior.
  Known limits: the select fallback takes the first keyword only, so a value whose leading token is not a plain keyword still shows a dash; background-position aliases cover the nine single-layer percentage forms; multi-layer background-position has no named preset.

- [x] 124 Move non-actionable page warnings out of the canvas top into a panel section
  Done: 2026-09-16 Task 124: `updateWarnings` (`src/renderer/js/main.js`) now keeps only the actionable "Not linked to shared stylesheet" warning above the canvas and collects the per-page notes — styleguide hint, blocked external scripts, duplicate ids and missing JS hooks — on `HE.pageIssues`, refreshing the panel only when the list changes. New `buildPageIssues` (`src/renderer/js/panel-sections.js`) renders them in a collapsed "Page issues" section, added to the Design panel (when an element is selected) and the Site scope (before Site check) only when there is at least one note. Smoke updated: duplicate id and CDN-script notes now assert the panel section and a clean canvas bar, plus a new check that the stylesheet-link warning keeps its Link action above the canvas. `npm test` 102 pass; smoke SMOKE OK with the documented pre-existing Typography font-picker check bypassed for the run, then restored.
  Goal: JS warnings/errors and per-page notes rendered as amber bars stacked at the top of the canvas, pushing the page down and making the editor feel broken; they are informational and belong out of the way.
  Files: `src/renderer/js/main.js`, `src/renderer/js/panel.js`, `src/renderer/js/panel-sections.js`, `src/renderer/js/smoke.js`, `TASKS.md`.
  Accept: nothing non-actionable renders above the canvas; issues appear in a collapsed "Page issues" section that hides when empty; the stylesheet-link warning and its Link button stay above the canvas; duplicate/external notes update live without a manual refresh; unit + smoke green.
  Don't: remove the per-page detection, the shared-stylesheet link action, or the Site check; do not auto-open the section or restyle unaffected chrome.
  Known limits: the section shows in Design (with a selection) and Site scopes only, and is capped at 5 duplicate ids / 5 missing selectors per page as before.
